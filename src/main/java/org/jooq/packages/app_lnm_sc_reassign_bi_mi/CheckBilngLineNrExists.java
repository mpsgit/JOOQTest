/**
 * This class is generated by jOOQ
 */
package org.jooq.packages.app_lnm_sc_reassign_bi_mi;


import java.math.BigDecimal;

import javax.annotation.Generated;

import org.jooq.Field;
import org.jooq.Parameter;
import org.jooq.Wetrn;
import org.jooq.impl.AbstractRoutine;
import org.jooq.packages.AppLnmScReassignBiMi;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.2"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class CheckBilngLineNrExists extends AbstractRoutine<String> {

	private static final long serialVersionUID = 197178710;

	/**
	 * The parameter <code>WETRN.APP_LNM_SC_REASSIGN_BI_MI.CHECK_BILNG_LINE_NR_EXISTS.RETURN_VALUE</code>.
	 */
	public static final Parameter<String> RETURN_VALUE = createParameter("RETURN_VALUE", org.jooq.impl.SQLDataType.VARCHAR, false);

	/**
	 * The parameter <code>WETRN.APP_LNM_SC_REASSIGN_BI_MI.CHECK_BILNG_LINE_NR_EXISTS.P_BILNG_SYS_ID</code>.
	 */
	public static final Parameter<BigDecimal> P_BILNG_SYS_ID = createParameter("P_BILNG_SYS_ID", org.jooq.impl.SQLDataType.NUMERIC, false);

	/**
	 * The parameter <code>WETRN.APP_LNM_SC_REASSIGN_BI_MI.CHECK_BILNG_LINE_NR_EXISTS.P_LINE_NR</code>.
	 */
	public static final Parameter<String> P_LINE_NR = createParameter("P_LINE_NR", org.jooq.impl.SQLDataType.VARCHAR, false);

	/**
	 * The parameter <code>WETRN.APP_LNM_SC_REASSIGN_BI_MI.CHECK_BILNG_LINE_NR_EXISTS.P_LINE_NR_GRP_ID</code>.
	 */
	public static final Parameter<BigDecimal> P_LINE_NR_GRP_ID = createParameter("P_LINE_NR_GRP_ID", org.jooq.impl.SQLDataType.NUMERIC, false);

	/**
	 * Create a new routine call instance
	 */
	public CheckBilngLineNrExists() {
		super("CHECK_BILNG_LINE_NR_EXISTS", Wetrn.WETRN, AppLnmScReassignBiMi.APP_LNM_SC_REASSIGN_BI_MI, org.jooq.impl.SQLDataType.VARCHAR);

		setReturnParameter(RETURN_VALUE);
		addInParameter(P_BILNG_SYS_ID);
		addInParameter(P_LINE_NR);
		addInParameter(P_LINE_NR_GRP_ID);
	}

	/**
	 * Set the <code>P_BILNG_SYS_ID</code> parameter IN value to the routine
	 */
	public void setPBilngSysId(Number value) {
		setNumber(P_BILNG_SYS_ID, value);
	}

	/**
	 * Set the <code>P_BILNG_SYS_ID</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setPBilngSysId(Field<? extends Number> field) {
		setNumber(P_BILNG_SYS_ID, field);
	}

	/**
	 * Set the <code>P_LINE_NR</code> parameter IN value to the routine
	 */
	public void setPLineNr(String value) {
		setValue(P_LINE_NR, value);
	}

	/**
	 * Set the <code>P_LINE_NR</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setPLineNr(Field<String> field) {
		setField(P_LINE_NR, field);
	}

	/**
	 * Set the <code>P_LINE_NR_GRP_ID</code> parameter IN value to the routine
	 */
	public void setPLineNrGrpId(Number value) {
		setNumber(P_LINE_NR_GRP_ID, value);
	}

	/**
	 * Set the <code>P_LINE_NR_GRP_ID</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setPLineNrGrpId(Field<? extends Number> field) {
		setNumber(P_LINE_NR_GRP_ID, field);
	}
}
